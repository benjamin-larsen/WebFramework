...
const methodsProxyHandler = {
    get(instance, prop) {
        ...
        if (methods === null || typeof methods !== "object" || methods.constructor !== Object) return undefined;
    ...
    ownKeys(instance) {
        ...
        const methods = instance.vnode.component.methods;
        if (methods === null || typeof methods !== "object" || methods.constructor !== Object) return [];
...
const instanceProxyHandler = {
    ...
    set(instance, prop, value) {
        if (!instance.vnode) throw Error("Instance is destroyed.")
        if (reservedProps.has(prop)) throw Error(`Can't set reserved property ${prop}.`);
    ...
    deleteProperty(instance, prop) {
        if (!instance.vnode) throw Error("Instance is destroyed.")
        if (reservedProps.has(prop)) throw Error(`Can't delete reserved property ${prop}.`);
        if (prop[0] === '$') throw Error(`Can't delete internal functions ${prop}.`);
